...
languages = ["DANISH", "ENGLISH", "ITALIAN", "POLISH"]
results = {}
...
def main():
    ...
    for lang in languages:
        solutions = read_solution_files(os.path.join(args.predictions, lang))
        truth = read_ground_truth_files(os.path.join(args.truth, lang))
    
        try:
            f1 = compute_score_multiple_predictions(truth, solutions, 'changes', labels=[0, 1])
        except KeyError:
            f1 = None
            print(f"No solution file found for one or more problems in {lang}")
    
        results[f"{lang}_f1_score"] = f1
